...
public class Database {
    ...
    public static final String CONNECTION_STRING="jdbc:sqlite:A:\\java\\Music_database\\"+MUSIC_DB;
...
public void close(){
    ...
    public List<String> queryOf_Albums_BY_Artist(String artistName,int sortOrder){
        //SELECT artists.name FROM albums INNER JOIN artists ON albums.artist=artists._id WHERE artists.name="Iron Maidan" ORDER BY albums.name COLLATE NOCASE ;
        StringBuilder sb=new StringBuilder(QWERY_OF_ALBLUM_BY_ARTIST);
        //using QWERY_OF_ALBLUM_BY_ARTIST  constant for below code
//        sb.append(TABLE_ALBUMS);
//        sb.append(".");
//        sb.append(COLOUMN_ALBUM_NAME);
//        sb.append(" FROM ");
//        sb.append(TABLE_ALBUMS);
...
//        sb.append(TABLE_ALBUMS);
//        sb.append(".");
//        sb.append(COLOUMN_ALBUM_ARTISTID);
//        sb.append(" = ");
//        sb.append(TABLE_ARTISTS);
//        sb.append(".");
//        sb.append(COLOUMN_ARTIST_ID);
//        sb.append(" WHERE ");
//        sb.append(TABLE_ARTISTS);
//        sb.append(".");
//        sb.append(COLOUMN_ARTIST_name);
...
//            sb.append(" COLLATE NOCASE ");
    ...
    public List<SongArtist> Qwery_Song_BY_Artist(String songName,int sortOrder){
        StringBuilder sb =new StringBuilder(QWERY_FOR_SONG_ARTIST);
        sb.append(songName);
        sb.append("\"");
        if (sortOrder!=ORDER_BY_NONE){
            sb.append(QWERY_SONG_ARTIST_SORT);
            if (sortOrder==ORDER_BY_DESC){
                sb.append(" DESC ");
            }else {
                sb.append(" ASC ");
        ...
        ResultSet result=statement.executeQuery(sb.toString())) {
            ...
            while (result.next()){
                ...
                songArtist.setSongtrack(result.getInt(3));
    ...
    public List<SongArtist> queryFor_Artist_BY_SONG_using_view(String songName){


        try{
            preparedStatementForSongArtist_View.setString(1,songName);
            ResultSet resultSet=preparedStatementForSongArtist_View.executeQuery();
